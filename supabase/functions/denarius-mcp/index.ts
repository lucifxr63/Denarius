import { MCP_PROTOCOL_VERSION, MCP_RESOURCE_METADATA, MCP_SERVER_INFO, TOOL_SCOPE, toolsForScopes, parseMcpRequest, rpcError, rpcResult } from './core.ts'
...
Deno.serve(async (httpRequest) => {
  ...
  let principalScopes=['financial:read']
  ...
  if (apiKey) {
    ...
    principalHeaders['x-denarius-scopes'] = principalScopes.join(',')
  } else {
    ...
    const admin=createClient(supabaseUrl,serviceRoleKey,{db:{schema:'cashflow'},auth:{persistSession:false}})
    const{data:grants,error:grantError}=await admin.rpc('resolve_denarius_mcp_oauth_grant',{p_owner_id:user?.id})
    const grant=grants?.[0]
    if(grantError||!grant)return response(403,{error:'denarius_connector_not_configured',setup_url:'https://denarius.scouttech.lat/connections'},origin)
    principalScopes=grant.scopes
    toolAuthorization=`Bearer ${serviceRoleKey}`
    principalHeaders['x-denarius-owner-id']=grant.owner_id
    principalHeaders['x-denarius-tenant-id']=grant.tenant_id
    principalHeaders['x-denarius-scopes']=principalScopes.join(',')
  }
  ...
  if (request.method === 'initialize') return response(200, rpcResult(request.id, {
    ...
    instructions: 'Denarius opera exclusivamente sobre la empresa autenticada. Las herramientas visibles dependen de los permisos concedidos y las escrituras requieren aprobación del usuario en Claude.',
  }))
  if (request.method === 'tools/list') return response(200, rpcResult(request.id, { tools: toolsForScopes(principalScopes) }))
  ...
  const requiredScope=TOOL_SCOPE[name as keyof typeof TOOL_SCOPE]
  if(!requiredScope||!principalScopes.includes(requiredScope))return response(200,rpcError(request.id,-32001,'Permission denied',{required_scope:requiredScope??'unknown'}))

  const toolResponse = await fetch(`${supabaseUrl}/functions/v1/denarius-tools`, {
    method: 'POST',
    headers: { Authorization: toolAuthorization!, apikey: serviceRoleKey, 'Content-Type': 'application/json', ...principalHeaders },
