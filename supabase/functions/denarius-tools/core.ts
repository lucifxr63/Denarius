export const TOOL_VERSION = '2026-08-10';
...
export interface ToolRequest {
  name: ToolName;
  arguments?: { horizon_days?: number; metric?: string; invoice_id?: string; delay_days?: number;title?:string;priority?:string;assignee?:string;due_date?:string;task_id?:string;status?:string };
}

export const TOOL_NAVIGATION: Record<ToolName,{path:string;label:string}> = {
  get_cash_position:{path:'/operations#financial-kpis',label:'Ver caja'},
  get_runway_and_burn:{path:'/operations#financial-kpis',label:'Ver indicadores'},
  list_overdue_invoices:{path:'/operations#overdue-invoices',label:'Ver facturas vencidas'},
  get_cash_projection:{path:'/operations#cash-projection',label:'Ver proyección'},
  get_restricted_cash:{path:'/operations#financial-kpis',label:'Ver caja restringida'},
  explain_metric:{path:'/dashboard',label:'Abrir dashboard'},
  get_financial_summary:{path:'/dashboard',label:'Abrir dashboard'},
  explain_projection_point:{path:'/operations#cash-projection',label:'Ver punto proyectado'},
  simulate_scenario:{path:'/operations#cash-projection',label:'Comparar escenario'},
  get_weekly_action_plan:{path:'/action-plan',label:'Abrir plan semanal'},
  get_weekly_action_outcome:{path:'/weekly-close',label:'Ver resultado semanal'},
  get_actionable_alerts:{path:'/alerts',label:'Ver alertas'},
  create_financial_action:{path:'/action-plan',label:'Ver acción creada'},
  update_financial_action_status:{path:'/action-plan',label:'Ver acción actualizada'},
};

export function toolEvidence(tool:ToolName,data:unknown,asOf:string){
  const record=(data&&typeof data==='object'&&!Array.isArray(data)?data:{}) as Record<string,unknown>;
  const items:Array<{label:string;value:string|number;as_of:string}>=[];
  const add=(label:string,key:string)=>{const value=record[key];if(typeof value==='number'||typeof value==='string')items.push({label,value,as_of:asOf})};
  if(Array.isArray(data)){items.push({label:'Registros encontrados',value:data.length,as_of:asOf});if(tool==='list_overdue_invoices')items.push({label:'Monto vencido total',value:data.reduce((sum,item)=>sum+Number((item as Record<string,unknown>).total_amount??0),0),as_of:asOf});}
  else if(tool==='get_cash_position'){add('Caja total','current_cash');add('Caja restringida','restricted_cash');add('Caja disponible','available_cash');}
  else if(tool==='get_runway_and_burn'){add('Burn mensual','monthly_burn');add('Runway en meses','runway_months');}
  else if(tool==='get_cash_projection'){add('Saldo mínimo','lowest_balance');add('Fecha del mínimo','lowest_date');add('Saldo final','ending_balance');}
  else if(tool==='get_financial_summary'){add('Caja actual','current_cash');add('Capital de trabajo','working_capital');add('Runway en meses','runway_months');}
  else if(tool==='get_restricted_cash'){add('Caja restringida','restricted_cash');}
  else if(tool==='explain_metric'){add('Métrica','metric');add('Valor','value');}
  else if(tool==='explain_projection_point'){const summary=record.summary as Record<string,unknown>|undefined;if(summary){for(const [label,key] of [['Saldo mínimo','lowest_balance'],['Fecha del mínimo','lowest_date']] as const){const value=summary[key];if(typeof value==='number'||typeof value==='string')items.push({label,value,as_of:asOf});}}}
  else if(tool==='simulate_scenario'){const result=record.result as Record<string,unknown>|undefined;if(result){for(const [label,key] of [['Saldo mínimo simulado','lowest_balance'],['Saldo final simulado','ending_balance']] as const){const value=result[key];if(typeof value==='number'||typeof value==='string')items.push({label,value,as_of:asOf});}}}
...
export const TOOL_JSON_SCHEMAS = {
  ...
  get_weekly_action_plan:{type:'object',additionalProperties:false,properties:{}},
  get_weekly_action_outcome:{type:'object',additionalProperties:false,properties:{}},
  get_actionable_alerts:{type:'object',additionalProperties:false,properties:{}},
  create_financial_action:{type:'object',additionalProperties:false,required:['title','priority','assignee','due_date'],properties:{title:{type:'string',minLength:3,maxLength:180},priority:{type:'string',enum:['LOW','MEDIUM','HIGH','URGENT']},assignee:{type:'string',minLength:1,maxLength:120},due_date:{type:'string',format:'date'}}},
  update_financial_action_status:{type:'object',additionalProperties:false,required:['task_id','status'],properties:{task_id:{type:'string',format:'uuid'},status:{type:'string',enum:['OPEN','IN_PROGRESS','BLOCKED','DONE']}}},
} as const;
