...
import { Check, XCircle, Trash2, Eye, EyeOff, FileText } from 'lucide-react';
...
export function InvoicesList({ invoices, onResolve, onDelete, ignoredIds, onToggleIgnore, canOperate=true, canDelete=true }: Props) {
  ...
  const btn = 'grid size-11 place-items-center rounded-md transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 disabled:opacity-50';

  return (
    <div className="rounded-2xl border border-border bg-card/60 p-5 backdrop-blur">
      <h3 className="mb-3 text-sm font-medium text-muted-foreground">Facturas</h3>
      {items.length === 0 ? (
        <OperationalEmptyState icon={FileText} title="Todavía no hay facturas" description="Registra una cuenta por cobrar o pagar para incorporarla a la proyección." actionLabel="Registrar primera factura" href="#quick-entry" />
      ) : (
            ...
            <li key={inv.id} className={`group flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 transition-colors hover:bg-muted/40 ${hidden ? 'opacity-40' : ''}`}>
              ...
              <span className="flex items-center gap-1.5">
                ...
                {onToggleIgnore && inv.status === 'PENDING' && (
                  <button
                    className={`${btn} ${hidden ? 'text-accent' : 'text-muted-foreground opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100'} hover:bg-muted`}
                ...
                <span className="flex items-center opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus-within:opacity-100">
                  {canOperate&&inv.status === 'PENDING' && (
                    <>
                      <button
